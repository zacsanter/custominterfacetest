        ...
        if (item.type === "speak" || item.type === "text") {
          ...
          for (let i = 0; i < formatted_messages.length; i++) {
            ...
            if (i < formatted_messages.length - 1) {
              // If the current or next message is bold, italic, underlined, or strikethrough, use one <br>
              if (/<b>|<i>|<u>|<strike>/.test(formatted_messages[i]) || /<b>|<i>|<u>|<strike>/.test(formatted_messages[i+1])) {
                combinedMessage += "<br>";
              } else {
