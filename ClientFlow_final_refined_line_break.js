      ...
      response.forEach((item) => {
        if (item.type === "speak" || item.type === "text") {
          ...
          const formatted_messages = item.payload.slate.content.map(contentItem => {
            ...
            // Wrap text in a div
            return `<div class="paragraphcustom">${text}</div>`;
          }).filter(text => text.trim() !== "" && text !== '<div class="paragraphcustom"><b></b></div>' && text !== '<div class="paragraphcustom"><i></i></div>');  // Filter out empty and unnecessary divs

          // Combine the div-wrapped paragraphs
