...
// Check if the item type is text
if (item.type === "text" && item.payload.message) {
    // Split the message into paragraphs and wrap each in <p></p> tags
    var paragraphs = item.payload.message.split('\n\n');
    var formattedMessage = "";
    paragraphs.forEach(function(para) {
        if (para.trim() !== "") {
            formattedMessage += "<p>" + para + "</p>";
        }
    });
    messageElement.innerHTML = formattedMessage;
} else {
    messageElement.innerText = item.payload.message;
}
