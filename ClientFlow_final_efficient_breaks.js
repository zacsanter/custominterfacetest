      ...
      response.forEach((item) => {
        if (item.type === "speak" || item.type === "text") {
          ...
          const formatted_messages = item.payload.slate.content.map(contentItem => {
            ...
            return text;
          }).filter(text => text.trim() !== "" && text !== "<b></b>" && text !== "<i></i>");  // Filter out empty and unnecessary texts

          // Efficiently combine these paragraphs with proper breaks
          let combinedMessage = "";
          for (let i = 0; i < formatted_messages.length; i++) {
            combinedMessage += formatted_messages[i];

            // Check for end of message or if the next message is not formatted
            if (i === formatted_messages.length - 1 || !/<b>|<i>|<u>|<strike>/.test(formatted_messages[i+1])) {
              if (i < formatted_messages.length - 1) {
                combinedMessage += "<br><br>";
              }
            } else {
              combinedMessage += "<br>";
            }
          ...
          messageElement.innerHTML = combinedMessage;
